...
import { connect, createRoom, type EventOfType, ip, waitForLobby, type WsClient } from './helpers.js';
...
// source. Curated sets are built from these known-ready ids (the e2e has no YOUTUBE_API_KEY, so new videos
// cannot be fetched — coverage of the metadata fetch lives in the unit tests).
const SEEDED = [
...
async function signInAdmin(request: APIRequestContext): Promise<void> {
  const res = await request.post('/api/auth/sign-in/email', { data: { email: ADMIN_EMAIL, password: ADMIN_PASSWORD } });
