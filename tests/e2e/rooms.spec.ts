...
type EventOfType<T extends ServerEvent['type']> = Extract<ServerEvent, { type: T }>;

// The API trusts X-Forwarded-For in this harness, so each test uses a distinct fake client IP to isolate
// its per-IP rate-limit and ban buckets.
function ip(label: string): Record<string, string> {
  return { 'x-forwarded-for': `198.51.100.${label}` };
}

async function createRoom(request: APIRequestContext, name: string, headers: Record<string, string>): Promise<{ code: string; sessionToken: string }> {
...
async function openWs(): Promise<WsClient> {
...
async function connect(token: string): Promise<{ ws: WsClient; playerId: string }> {
...
async function waitForLobby(ws: WsClient, predicate: (lobby: EventOfType<'lobby'>['lobby']) => boolean): Promise<EventOfType<'lobby'>['lobby']> {
  for (;;) {
