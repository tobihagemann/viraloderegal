...
- `apps/web` — Vue 3 (`<script setup>`) SPA built with Vite. The game UI and a route-guarded `/admin` section.
- `apps/api` — Hono API on Node 22 (`@hono/node-server`): Kysely + Postgres, Zod-validated requests; the single database client. It owns the WebSocket hub and the authoritative game loop.
...
- **Server-authoritative game loop.** A single long-lived process owns phase timers and the hidden answer until reveal. The hidden answer is not just the view count: video-identifying metadata a player could search to look up that count (the title, and any channel/thumbnail added later) is part of it too, so it rides the reveal payload and never the clip/guess-phase payloads. Only what the clip embed strictly needs (`youtubeId`, clip bounds) is sent earlier. `rounds.current_phase`/`phase_end_at` persist on every transition; per-room commands serialize through an in-memory mutex (`apps/api/src/rooms/roomLock.ts`). That mutex is **non-reentrant** — never nest `withRoomLock` for the same room (the inner acquire awaits a tail containing the outer and deadlocks). One boundary per room: a caller acquires the lock and invokes lock-free internals; timer/lifecycle callbacks that have no outer lock acquire it themselves, while code already running under the lock (e.g. `handleJoin` → resume/reschedule paths) must call lock-free helpers. Horizontal scaling is out of scope.
...
- **i18n / UI.** The UI is German, implemented i18n-ready (vue-i18n), responsive for Chrome desktop and mobile, on a `red-600`-family accent. Icons come from `@heroicons/vue`'s Micro set (`@heroicons/vue/16/solid`, 16px → `size-4`, colored via `text-*`) — don't hand-author inline SVG icons; icon-only action buttons use the shared `icon-btn` + `icon-btn-accent`/`icon-btn-neutral` utilities (`apps/web/src/styles/main.css`) and carry an `aria-label`.
- **Testing layers.** CI runs Vitest (`pnpm test`) _before_ `migrate`, so the schema is not applied during Vitest — keep Vitest DB-free (drive the app in-memory via `app.request()`, unit-test pure helpers). DB-backed coverage belongs in Playwright `e2e`, which runs after `migrate`. Vitest runs only in `packages/shared` and `apps/api`; `apps/web` has no Vitest setup, so a web-side invariant that must be enforced in CI belongs in a compile-time check caught by `vue-tsc` (run by the web `build`/`typecheck`, both in the merge gate), not a unit test — or stand up Vitest in `apps/web` first.
