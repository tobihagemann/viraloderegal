...
- **Server-authoritative game loop.** A single long-lived process owns phase timers and the hidden answer until reveal. `rounds.current_phase`/`phase_end_at` persist on every transition; per-room commands serialize through an in-memory mutex. Horizontal scaling is out of scope.
